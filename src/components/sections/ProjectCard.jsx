    ...
    return (
      <div className="relative group flex flex-col bg-[#1a2a3d] rounded-lg shadow-lg overflow-hidden transition-transform duration-500 ease-in-out hover:scale-105 hover:shadow-xl">
        ...
        <div className="project-info p-4 flex flex-col justify-between flex-grow relative z-20">
          ...
          <div className="tech-stack text-sm text-gray-700">
            {project.techStack.map((tech, index) => (
              <span key={index} className="mr-2 inline-block bg-red-200 px-3 py-1 rounded-full">
                {tech}
              </span>
            ))}
          </div>
        ...
        <div className="absolute inset-0 border-4 border-transparent bg-gradient-to-r from-red-400 to-orange-800 opacity-0 group-hover:opacity-50 transition-opacity duration-500 ease-in-out z-0"></div>
      </div>
    );
  };
